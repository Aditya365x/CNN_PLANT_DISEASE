...
def main():
    ...
    selected_oilseed = st.selectbox("🌾 Select oilseed crop", oilseed_options, index=0)
    ...
    if uploaded_image is not None:
        ...
        if st.button("🔮 Predict Disease & Suggest Cure"):
            ...
            st.info(f"🌾 Selected oilseed crop: **{selected_oilseed}**")

            # Step 2: RAG LLM for preventive measures & cure
            with st.spinner("Querying knowledge base & generating preventive measures..."):
                try:
                    # 🔽 CHANGED: pass crop + disease to LLM
                    preventive_text = get_preventive_measures(selected_oilseed, predicted_disease)
                except Exception as e:
