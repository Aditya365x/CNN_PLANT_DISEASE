...
# ==========================================================
def get_preventive_measures(crop_name: str, disease_name: str) -> str:
    """
    Main function you call from `app.py`.
    ...
    question = (
        ...
        f"in the oilseed crop: '{crop_name}'. "
        f"Using ONLY the information from the PDF knowledge base, explain this specific "
        f"disease in this specific crop and provide:\n"
        f"1) Key symptoms\n"
        f"2) Preventive agronomic practices and field hygiene for {crop_name}\n"
        f"3) Recommended treatments or controls for {disease_name} in {crop_name} "
        f"   (biological and chemical, if mentioned)\n"
        f"4) Any practical, farmer-friendly tips.\n\n"
        f"If the documents do not have precise information for this crop–disease pair, "
        f"clearly state that information is not available instead of guessing."
    )
